...
def open_csv():
    ...
    #df.rename(columns={"Count_per_Branche_x": "Anzahl Stellen pro Branche"}, inplace=True)
    df.info()
    return df
...
def question1(df):
    ...
    df_na = df.dropna(axis = 0, subset = "Branche")
    grouped_branche = df_na.groupby('Branche')
    abs_branche = grouped_branche.size().sort_values(ascending=False)
    # Or:# abs= df_na.groupby(['Branche'])["Branche"].count().sort_values(ascending=False)
    #print(abs)
    ...
    df2 = df_na.copy()
    df2['Count_per_Branche'] = df_na.groupby('Branche')['Branche'].transform('count')
...
def question2(df):
    ...
    #Matplotlib Graphics
    # plot_canton = df
    # plot_canton.hist(figsize=(15, 15))
    # plt.pyplot.show()
    # #plt.show()
...
def question3(df):
    ...
    df_na = df.dropna(axis = 0, subset = "Arbeitgeber_Bewertung")
    mean_rating = pd.Series.mean(df_na['Arbeitgeber_Bewertung'])
...
def question5(df):
    '''
    #Question 5: Wie ist die Verteilung bei den offenen Stellen in Bezug auf Führungsposition?
    :param df: df, welches aus JobAgent_stage.csv erzeugt wird.
    :return: df für Excel
    '''
    print(40 * "*")
    print("Question 5: Wie ist die Verteilung bei den offenen Stellen in Bezug auf Führungsposition? ?")

    # Group by Branche mit absoluten Zahlen:
    df_na = df.dropna(axis=0, subset="Führungsposition")
    grouped_führung = df_na.groupby('Führungsposition')
    abs_führung = grouped_führung.size().sort_values(ascending=False)
    # Or:# abs= df_na.groupby(['Branche'])["Branche"].count().sort_values(ascending=False)
    # print(abs)


    # Count per Führungsposition als neue Variable:
    df2 = df_na.copy()
    df2['Count_per_Führungsposition'] = df_na.groupby('Führungsposition')['Führungsposition'].transform('count')
    # df2['CUMSUM'] = df['JobNr'].cumsum() #cumsum

    # Group by Branche mit Relativen Zahlen:
    sum_führung = grouped_führung.size().sum()
    df2["In % von Total"] = round((df2["Count_per_Führungsposition"] / sum_führung) * 100, 2)

    rel_führung = df2.groupby(['Führungsposition'])["In % von Total"].mean().sort_values(ascending=False)

    # Concat Result:
    result_führung= pd.concat([abs_führung, rel_führung], axis=1)
    result_führung.rename(columns={0: "Anzahl offener Stellen"}, inplace=True)
    print("Sachbearbeiter und Fachverantwortung sind ungefähr gleich verteilt. Stelen mit Führungsposition sind nicht nennenswert vorhanden")
    print(result_führung)

    return result_führung
...
def question6(df):
 ...
 #Jobs_concat = Jobs_concat.rename(index={"": "JobNr"})
...
def create_xlsx(result, question_number):
    ...
    if int(question_number) == 1:
        ...
        with pd.ExcelWriter(f"Result_Question_{question_number}_group.xlsx") as writer:
        ...
        with pd.ExcelWriter(f"Result_Question_{question_number}_group.xlsx") as writer:
            df.to_excel(writer, sheet_name=str("Führungsposition"))
